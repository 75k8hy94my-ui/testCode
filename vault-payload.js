const DATA_KEYS = {
  ...
  homeCards: 'mangaReaderHomeCards', study: 'mangaReaderStudy', indexSearchSettings: 'mangaReaderIndexSearchSettings', roppoState: 'mangaReaderRoppoState'
};
...
const defaults = { folders: [], items: [], videos: [], videoFolders: [], videoMeta: {}, authorCards: [], mangaInfo: {}, toc: {}, lastPages: {}, theme: 'dark', dashboardVisibility: { mobile: { ...defaultDashboardVisibility }, desktop: { ...defaultDashboardVisibility } }, homeCards: defaultHomeCards.slice(), study: createEmptyStudy(), indexSearchSettings: cloneIndexSearchSettings(), roppoState: cloneRoppoState() };
...
const normalizeStringArray = (value) => Array.isArray(value) ? value.map((item) => String(item ?? '').trim()).filter(Boolean) : [];
function normalizeIndexSearchSettings(value) {
  const source = isObject(value);
  const modes = isObject(source.matchModes);
...
const api = { DATA_KEYS, defaults, normalize, buildFromLocalStorage: buildFromStorage, applyToLocalStorage: applyToStorage, clearDeviceData, normalizeIndexSearchSettings, normalizeRoppoState };
