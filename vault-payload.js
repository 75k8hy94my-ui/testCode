const DATA_KEYS = {
  ...
  homeCards: 'mangaReaderHomeCards', study: 'mangaReaderStudy', indexSearchSettings: 'mangaReaderIndexSearchSettings', statuteNotes: 'mangaReaderStatuteNotes'
};
...
function normalizeStatuteNotes(value) {
  ...
  Object.entries(source).forEach(([key, note]) => {
    ...
    if (!normalizedKey || typeof note !== 'string') return;
    const text = note.replace(/\r\n?/g, '\n');
    if (text.trim()) result[normalizedKey] = text;
  });
  return result;
}
