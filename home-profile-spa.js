...
function renderProfile(){
  ...
  target.innerHTML='<section class="profileContent"><p id="homeSyncStatus" class="syncStatus profileToastStatus" aria-live="polite"></p><h2>プロフィール設定</h2><p class="profileLead">アカウントと、端末／クラウドに残るデータの扱いです。</p><dl class="profileMeta"><div><dt>ログイン中</dt><dd id="profileEmail"></dd></div></dl><section class="profileCard"><h3>保管庫</h3><p>本棚・動画・作者カードなどのデータはブラウザ内で暗号化して同期します。</p><a class="glassBtn profileAction" href="sync.html">保管庫を開く</a></section><section class="profileCard"><h3>VPN診断</h3><p>「VPNではない」と固定したIPアドレスを管理します。</p><div id="profileNonVpnIps"></div></section><section class="profileCard"><h3>表示</h3><label class="profileCheck"><input id="profileThemeLight" type="checkbox">ライトテーマ</label></section><section class="profileCard"><h3>セッション</h3><p>ログアウトすると、この端末の保管庫データを消します。</p><button class="glassBtn profileDanger" id="profileLogoutBtn" type="button">ログアウト</button></section></section>';
