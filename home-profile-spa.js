...
const SPA_PAGES=(window.AppShell&&Array.isArray(AppShell.SPA_PAGES)?[...AppShell.SPA_PAGES]:['home.html','profile.html','index-search.html','hyakusen.html','links.html','manga.html','video.html','reader.html']);
...
function routeName(path=location.pathname){const name=path.split('/').pop();if(name==='profile.html')return'profile';if(name==='index-search.html')return'index-search';if(name==='hyakusen.html')return'hyakusen';if(name==='links.html')return'links';if(name==='manga.html')return'manga';if(name==='video.html')return'video';if(name==='reader.html')return'reader';return'home';}
function setTitle(route){const titles={home:'ホームズ',profile:'プロフィール設定','index-search':'索引検索',hyakusen:'判例百選',links:'リンク管理',manga:'漫画',video:'動画',reader:'漫画リーダー'};const title=titles[route]||titles.home;document.title=title;const h1=document.getElementById('shellTitle');if(h1)h1.textContent=title;}
...
function syncHeaderRoute(){
  ...
  document.querySelectorAll('.topActions a[href], .globalShellNav a[href]').forEach((link)=>{
    ...
    const active=(route==='home'&&name==='home.html')||(route==='manga'&&name==='manga.html')||(route==='video'&&name==='video.html')||(route==='index-search'&&name==='index-search.html')||(route==='hyakusen'&&name==='hyakusen.html')||(route==='links'&&name==='links.html');
...
async function renderReader(route=routeName(),generation=renderGeneration){
  ...
  try{
    ...
    const runtime=window.ReaderRouteRuntimeFactory.create({
      ...
      fetchReader:()=>fetch('reader.html?v=20260916-reader',{cache:'no-store'}),
...
function renderRoute(){ensureAppShell();const route=routeName(),generation=++renderGeneration,app=document.getElementById('homeApp');document.documentElement.classList.toggle('reader-entry-manga',route==='manga');document.documentElement.classList.toggle('reader-entry-video',route==='video');if(app){app.classList.toggle('reader-route',['manga','video','reader'].includes(route));app.dataset.readerRoute=route;}if(route!=='video')cleanupVideoRoute();if(!['manga','video','reader'].includes(route))cleanupReaderRuntime();else if(route!=='manga'&&route!=='video')cleanupMangaRoute();if(route==='profile')renderProfile();else if(route==='index-search')renderIndexSearch();else if(route==='hyakusen')renderHyakusen();else if(route==='links')renderLinks();else if(route==='manga')renderManga(generation);else if(route==='video')renderVideo(generation);else if(route==='reader')renderReader(route,generation);else renderHome();document.dispatchEvent(new CustomEvent('home-profile-routechange',{detail:{route}}));}
