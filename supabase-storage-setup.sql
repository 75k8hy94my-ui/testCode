...
drop policy if exists "Users can insert vault assets" on storage.objects;
drop policy if exists "Users can select vault assets" on storage.objects;
drop policy if exists "Users can delete vault assets" on storage.objects;
create policy "Users can insert vault assets" on storage.objects for insert to authenticated
with check (bucket_id = 'vault-assets' and (storage.foldername(name))[1] = (select auth.uid()::text));
create policy "Users can select vault assets" on storage.objects for select to authenticated
using (bucket_id = 'vault-assets' and (storage.foldername(name))[1] = (select auth.uid()::text));
create policy "Users can delete vault assets" on storage.objects for delete to authenticated
using (bucket_id = 'vault-assets' and (storage.foldername(name))[1] = (select auth.uid()::text));
