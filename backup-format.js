...
function normalizeBackupStatuteNotes(value) {
  const result = {};
  Object.entries(isBackupObject(value) ? value : {}).forEach(([key, note]) => {
    if (!isBackupObject(note)) return;
    const noteText = text(note.text); if (!noteText) return;
    result[key] = { text: noteText, updatedAt: Number(note.updatedAt) > 0 ? Number(note.updatedAt) : Date.now(), tags: normalizeBackupStringArray(note.tags) };
  });
  return result;
}
...
function normalizeData(data) {
  ...
  return {
    ...
    homeCards: Array.isArray(x.homeCards) ? x.homeCards.map((id) => text(id)).filter(Boolean) : DEFAULT_HOME_CARDS.slice(), study: normalizeBackupStudy(x.study), indexSearchSettings: normalizeBackupIndexSearchSettings(x.indexSearchSettings), roppoState: normalizeBackupRoppoState(x.roppoState), statuteNotes: normalizeBackupStatuteNotes(x.statuteNotes)
  };
}
