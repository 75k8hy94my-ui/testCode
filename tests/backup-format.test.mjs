...
test('versioned backup round-trips author cards and supplies home and study defaults', () => {
  ...
  assert.deepEqual(migrateBackup(result), { folders: data.folders, items: [], videos: [], authorCards: data.authorCards, mangaInfo: {}, toc: {}, lastPages: {}, theme: 'light', dashboardVisibility: { mobile: { continue: false, 'recent-added': false, 'recent-read': false, unread: false, random: false, favorites: false }, desktop: { continue: false, 'recent-added': false, 'recent-read': false, unread: false, random: false, favorites: false } }, homeCards: defaultHomeCards, study: emptyStudy });
});

test('backup v2 preserves home cards and study while legacy v2 gets defaults', () => {
  const result = createBackup({ homeCards: ['study', 'bookshelf'], study: { preferences: { autoSpeak: true } } }, '2026-08-26T00:00:00Z');
