(function (root, factory) {
  ...
  function deliveryStateFromResult(result) {
    if (!result) return 'waiting';
    if (result.status === 'added') return 'synced';
    if (result.status === 'duplicate') return 'duplicate';
    if (result.status === 'locked') return 'locked';
    return 'waiting';
  }

  function makeQueue(store) { return {
    async list() { const value = await store.get(KEYS.pending); return Array.isArray(value) ? value : []; },
    async enqueue(draft) { const items = await this.list(); const id = uid(); items.push({ id, draft, queuedAt: Date.now() }); await store.set(KEYS.pending, items); return id; },
    async ack(id) { const items = await this.list(); await store.set(KEYS.pending, items.filter((item) => item.id !== id)); }
  }; }
  ...
  function makeSerializedFlusher(work) {
    ...
    return function run(key) {
      ...
      let promise; try { promise = Promise.resolve(work(key)); } catch (error) { promise = Promise.reject(error); }
      promise = promise.finally(() => { if (inFlight.get(normalizedKey) === promise) inFlight.delete(normalizedKey); });
      inFlight.set(normalizedKey, promise); return promise;
    };
  }
  ...
  async function flushPending(chromeApi, queue, tabId) {
    ...
    if (!tabs.length) return { delivered:0, lastResult:null };
    ...
    for (const item of await queue.list()) {
      ...
      for (const tab of tabs) {
        try {
          ...
          if (response) lastResult = response;
  ...
  function installChromeHandlers(chromeApi) {
    ...
    chromeApi.runtime.onMessage.addListener((message, sender, sendResponse) => {
      (async () => {
        ...
        if (message.type === 'QUEUE_DRAFT') { const id=await queue.enqueue(message.draft); const result=await serialFlush('delivery'); return { ok:true, id, delivered:result.delivered>0, deliveryState:deliveryStateFromResult(result.lastResult), lastResult:result.lastResult }; }
        if (message.type === 'TESTCODE_READY' || message.type === 'FLUSH_PENDING') return Object.assign({ok:true},await serialFlush('delivery'));
        return { ok:false,error:'unknown-message' };
      })().then(sendResponse).catch((error)=>sendResponse({ok:false,error:String(error&&error.message||error)})); return true;
    });
