(() => {
  ...
  markerToggle.addEventListener('click', () => { markerPanel.hidden = !markerPanel.hidden; renderMarkers(); }); add.addEventListener('click', () => { const seconds = Number(secondsInput.value); const label = labelInput.value.trim(); if (!Number.isFinite(seconds) || seconds < 0 || !label) return; markers.push({ seconds, label }); save(markers); secondsInput.value = ''; labelInput.value = ''; renderMarkers(); });
